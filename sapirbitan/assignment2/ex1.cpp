...
node* find_key(node& root, int& key) {
	...
	if (found_key != NULL)
		return found_key;
	return NULL;// if we haven't found anything - return NULL
}

//this function prints the key's ancestors
void print_anc(node& key){
	if (key.ancestor == NULL) {//since the only node that has no ancestor is the root
		cout << "root does not have ancestors" << endl;
	}
	node current_node = key;
	while (current_node.ancestor != NULL) {
